...
const MatchSummary = () => {
  ...
  const resultClass = didUserWin
    ? "text-green-400"
    : isDraw
    ? "text-yellow-400"
    : "text-red-400";
  const resultText = didUserWin ? "Victory" : isDraw ? "Draw" : "Defeat";
  const resultIcon = didUserWin ? (
  ...
  return (
    <div className="animate-fade-in">
        ...
        <CardContent className="p-6">
          <div className="space-y-6">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <div className="text-lg font-semibold">
                  {formatStat(homeStats.shots)}
                </div>
                <div className="text-sm text-gray-400">Shots</div>
                <div className="text-lg font-semibold">
                  {formatStat(awayStats.shots)}
                </div>
              </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <div className="text-lg font-semibold">
                  {formatStat(homeStats.shotsOnTarget)}
                </div>
                <div className="text-sm text-gray-400">Shots on Target</div>
                <div className="text-lg font-semibold">
                  {formatStat(awayStats.shotsOnTarget)}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <StatBar
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                <div className="text-xl font-semibold">
                  {formatStat(homeStats.yellowCards)}
                </div>
                ...
                <div className="text-xl font-semibold">
                  {formatStat(awayStats.yellowCards)}
                </div>
              </div>
      ...
      {/* Reward card */}
      <Card
        className={`mb-6 bg-footbai-container ${
          didUserWin ? "border-green-500/30" : "border-footbai-header"
        }`}
      >
        <CardHeader
          className={`${
            didUserWin ? "bg-green-500/10" : "bg-footbai-header"
          } pb-3`}
        >
          ...
          {didUserWin ? (
            <div className="text-center">
              ...
              <p className="text-sm text-gray-300">
                Use these points to upgrade your team attributes and become even
                stronger.
              </p>
            </div>
          ) : (
            <div className="text-center">
              <div className="text-lg mb-2">
                {isDraw ? "It's a draw!" : "Better luck next time!"}
              </div>
              <div className="flex justify-center items-center space-x-2 my-4">
